...
console.debug("Loading Auth controller");
...
route("post", "/auth/login", [
  ...
  async (ctx) => {
    ...
    if (token) {
      ...
      console.debug(`Login failed for user: ${username}`);
      ctx.status = 401;
...
route("post", "/auth/register", [
  ...
  async (ctx) => {
    console.debug("Handling registration request");
    const { username, password } = ctx.request.body;
    console.debug(`Registration attempt for user: ${username}`);
    try {
...
route("get", "/auth/me", [
  ...
  (ctx) => {
    console.debug(
      "Handling get current user request for user:",
      ctx.state.user,
    );
